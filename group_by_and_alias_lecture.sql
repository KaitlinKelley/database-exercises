...
# This also will return the count of each of those names (in the entire database)
...
SELECT hire_date, COUNT(*)   #show the hire dates, and the number of occurrences of that hire date
FROM employees
GROUP BY hire_date           #show only the unique hire dates, no duplications
ORDER BY COUNT(*) DESC       #list in descending order by the number of occurrences of each hire date
LIMIT 10;
